...
struct STE {
    ...
    string literalValue;  //如果为字面值则存储于此
};
...
struct SymbolTable{
     ...
     */
    string get_scoped_name(const string& id) const;
     ...
     * @return Operand 
     */
    ir::Operand get_operand(const string& id) ;
...
struct Analyzer {
    ...
    void analysisBType(BType *, ir::Type &);

    void analysisConstDef(ConstDef *, vector<ir::Instruction *> &, Type&);

    void analysisConstInitVal(ConstInitVal *, vector<ir::Instruction *> &, vector<ir::Operand>&);

    void analysisVarDecl(VarDecl *, vector<ir::Instruction *> &);

    void analysisVarDef(VarDef *, vector<ir::Instruction *> &, Type&);

    void analysisInitVal(InitVal *, vector<ir::Instruction *> &, vector<ir::Operand>&);

    void analysisFuncType(FuncType *, ir::Type &);

    void analysisFuncFParam(FuncFParam *, ir::Operand &);
    ...
    void analysisLVal(LVal *, vector<ir::Instruction *> &, string&, ir::Operand&);
    ...
    void analysisUnaryOp(UnaryOp *, TokenType &);

    void analysisFuncRParams(FuncRParams *, vector<ir::Instruction *> &, vector<ir::Operand> &);
